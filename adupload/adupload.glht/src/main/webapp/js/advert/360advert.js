...
$(function () {
    $("#jqGrid").jqGrid({
        multiselect: false,
        url: '../advert/advert/list?serviceType=2',
        postData : vm.q,
        colModel: [{
            label: '操作',
            width: 25,
    ...
    function operateFormat(value, options, row) {
        ...
        var lowValue = _.toLower(row.sourceExtension);
        if ((row.uploadUserId == parent.vm.user.userId || parent.vm.user.isHost == true) && lowValue == "zip" ) {
    ...
    function projectNameFormat(value, options, row) {
        return row.sourceId + '<br/>' + sprintf('<a style="border-bottom: 1px dashed #16a085;" href="javascript:vm.projectNamePreview(%s,%s)">%s</a>', "'" + row.projectName + "'", "'" + row.serviceType + "'", row.projectName)
    }
        ...
        operation: function (type) {
            if (type === "zip") {
                var index = layer.open({
                    title: '360全景广告 - ZIP上传<span class="text-navy">' + vm.titleSuffix + '</span>',
                    type: 2,
                    closeBtn: 2,
                    content: '/advert/360advertZip.html',
                    area: ['90%', '90%'],
                ...
                var index = layer.open({
                    title: '360全景广告 - 图片批量上传<span class="text-navy">' + vm.titleSuffix + '</span>-支持 *.png、*.jpg、*.swf、*.gif 格式文件',
                    type: 2,
                    closeBtn: 2,
                    content: '/advert/360advertBatch.html',
                    area: ['90%', '70%'],
                    end: function () {
                        vm.reload();
                    }
        ...
        preview: function (comment) {
            ...
            var index = parent.layer.open({
                ...
                success: function (layero, index) {
                    ...
                    iframeWin.vm.q.serviceType = vm.file.serviceType;
                    iframeWin.vm.q.myOwn = vm.q.myOwn;
                    iframeWin.vm.comment = comment;
                    iframeWin.vm.preSelectShow = false;
                    iframeWin.vm.initSingleBody();
                }
            });
        },
        projectNamePreview: function (projectName, serviceType) {
            var index = parent.layer.open({
                title: false,
                type: 2,
                closeBtn: 2,
                ...
                success: function (layero, index) {
                    ...
                    iframeWin.vm.q.serviceType = serviceType;
                    iframeWin.vm.q.myOwn = vm.q.myOwn;
